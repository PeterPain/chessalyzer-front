...
;(function() {
	...
	function constructor(containerElOrString, config) {
		...
		widget.drawHeatmap = function(map, min, max, config = {}) {
			...
			// disable board
			if (cfg.disableSquares) disableSquares()
			...
			if (max !== 0) {
				for (let i = 0; i < 8; i++) {
					for (let j = 0; j < 8; j++) {
						...
						let col = COLUMNS[j]
						let row = 8 - i
						let alpha = cfg.scaling(map[i][j], max).toFixed(2)
						let value = map[i][j].toFixed(2)
						let tile = $('#heat-' + squareElsIds[col + row])
						let animTime =
							(cfg.animTime - Math.pow(alpha, 4)) * 1000
						tile.animate(
							{
		...
		function checkHeatmapConfig(config) {
			const cfg = {}
			cfg.color = Object.prototype.hasOwnProperty.call(config, 'color')
				? config.color
				: [255, 128, 0]

			cfg.disableSquares = Object.prototype.hasOwnProperty.call(
				config,
				'disableSquares'
			)
				? config.disableSquares
				: true

			cfg.animTime = Object.prototype.hasOwnProperty.call(
				config,
				'animTime'
			)
				? config.animTime
			...
			cfg.unit = Object.prototype.hasOwnProperty.call(config, 'unit')
				? config.unit
				: ''

			cfg.scaling = Object.prototype.hasOwnProperty.call(
				config,
				'scaling'
			)
				? config.scaling
				: (val, max) => {
						return Math.sqrt(val / max)
				  }
